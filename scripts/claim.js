...
const { attachNFTContract,attachStakeTicket, readConfig, sleep} = require('./utils/helper')
...
const main = async () => {
    ...
    let nftContract = await attachNFTContract(deployer.address, erc721Address)

    let stakeSticketAddress = await readConfig("1", "STAKE_TICKET_ADDRESS")
    let stakeSticket = await attachStakeTicket(deployer.address, stakeSticketAddress)

    let tx = await nftContract.setMinterRole(stakeSticketAddress)
    console.log("setMinerRole tx.hash", tx.hash)
    await sleep(10000)

    let elaHash="78c1645758228af7255c596cdc276d95ce47b52533b0bf14bd0136cf61560f01"
